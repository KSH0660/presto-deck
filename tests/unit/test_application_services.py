...
class TestDeckService:
    ...
    async def test_create_deck_success(
        self, deck_service, deck_creation_request, test_user_id
    ):
    ...
    async def test_create_deck_with_minimal_request(self, deck_service, test_user_id):
        ...
        minimal_request = DeckCreationRequest(title="Minimal Deck", topic="Basic Topic")
    ...
    @pytest.mark.asyncio
    async def test_get_deck_unauthorized(
        self, deck_service, sample_deck, another_user_id
    ):
    ...
    async def test_get_deck_with_slides(self, deck_service, sample_deck, test_user_id):
        ...
        deck, retrieved_slides = await deck_service.get_deck_with_slides(
            sample_deck.id, test_user_id
        )
    ...
    async def test_list_decks(self, deck_service, test_user_id):
        ...
        decks = []
        for i in range(5):
            deck = Deck(user_id=test_user_id, title=f"Deck {i + 1}")
            await deck_service.deck_repo.create(deck)
            decks.append(deck)

        # Create deck for different user
        other_deck = Deck(user_id="other-user", title="Other User's Deck")
    ...
    @pytest.mark.asyncio
    async def test_cancel_deck_generation_success(
        self, deck_service, sample_deck, test_user_id
    ):
        ...
        updated_deck = await deck_service.cancel_deck_generation(
            sample_deck.id, test_user_id
        )
        ...
        deck_service.cache_manager.set_cancellation_flag.assert_called_once_with(
            sample_deck.id
        )
    ...
    @pytest.mark.asyncio
    async def test_cancel_deck_invalid_status(
        self, deck_service, sample_deck, test_user_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_deck_unauthorized(
        self, deck_service, sample_deck, another_user_id
    ):
    ...
    async def test_get_deck_events(self, deck_service, sample_deck, test_user_id):
        ...
        # Get all events
        retrieved_events = await deck_service.get_deck_events(
            sample_deck.id, test_user_id
        )
        ...
        recent_events = await deck_service.get_deck_events(
            sample_deck.id, test_user_id, from_version=3
        )
    ...
    @pytest.mark.asyncio
    async def test_get_deck_events_unauthorized(
        self, deck_service, sample_deck, another_user_id
    ):
    ...
    async def test_publish_event_error_handling(
        ...
        deck_service.stream_publisher.publish_event.side_effect = Exception(
            "Redis error"
        )
...
class TestSlideService:
    ...
    async def test_get_slide_success(
        self, slide_service, sample_deck, sample_slide, test_user_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_slide_unauthorized(
        self, slide_service, sample_deck, sample_slide, another_user_id
    ):
    ...
    async def test_update_slide_success(
        self, slide_service, completed_deck, sample_slide, test_user_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_slide_invalid_deck_status(
        self, slide_service, sample_deck, sample_slide, test_user_id
    ):
        ...
        with pytest.raises(InvalidDeckStatusException):
            await slide_service.update_slide(
                sample_slide.id, "Update prompt", test_user_id
            )
    ...
    @pytest.mark.asyncio
    async def test_add_slide_unauthorized(
        self, slide_service, completed_deck, another_user_id
    ):
        ...
        with pytest.raises(UnauthorizedAccessException):
            await slide_service.add_slide(
                completed_deck.id, 1, "Prompt", another_user_id
            )
    ...
    async def test_create_slide_internal(self, slide_service, completed_deck):
        ...
        html_content = (
            "<h1>New Slide</h1><p>Content with <script>alert('xss')</script></p>"
        )
        ...
        with patch("app.application.services.html_sanitizer") as mock_sanitizer:
            mock_sanitizer.sanitize.return_value = (
                "<h1>New Slide</h1><p>Content with </p>"
            )
    ...
    @pytest.mark.asyncio
    async def test_update_slide_content_internal(
        self, slide_service, completed_deck, sample_slide
    ):
    ...
    async def test_service_error_handling(
        self, slide_service, sample_deck, test_user_id
    ):
        """Test service error handling and logging."""
        # Mock stream publisher to raise exception
        slide_service.stream_publisher.publish_event.side_effect = Exception(
            "Publishing failed"
        )
...
class TestServiceIntegration:
    ...
    async def test_deck_and_slide_service_integration(self, test_user_id):
        ...
        request = DeckCreationRequest(
            title="Integration Test Deck", topic="Testing service integration"
        )
        deck = await deck_service.create_deck(request, test_user_id)
        ...
        with patch("app.application.services.html_sanitizer") as mock_sanitizer:
            ...
            slide = await slide_service.create_slide(
                deck_id=deck.id, slide_order=1, html_content="<h1>Integration Test</h1>"
            )

        # Retrieve deck with slides using DeckService
        retrieved_deck, slides = await deck_service.get_deck_with_slides(
            deck.id, test_user_id
        )
