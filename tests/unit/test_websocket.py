...
from app.api.schemas import Event, SlideUpdateRequest, SlideAddRequest
from app.api.websocket import WebSocketManager, _handle_client_message, _send_event_replay
...
class TestWebSocketManager:
    ...
    @pytest.mark.asyncio
    async def test_connect_first_connection(self, ws_manager, mock_websocket, mock_redis_pubsub):
        """Test connecting the first WebSocket for a deck."""
        deck_id = uuid4()
        user_id = "test-user-123"
        
        with patch('app.api.websocket.metrics.record_websocket_connection') as mock_metrics:
    ...
    async def test_consume_deck_events_error(self, ws_manager, mock_redis_pubsub):
        ...
        mock_redis_pubsub.subscribe_to_deck_channel.side_effect = Exception("Redis error")
...
class TestWebSocketEndpoint:
    ...
    async def test_websocket_endpoint_authentication_failure(self, mock_websocket):
        ...
        with patch('app.api.websocket.security_service.extract_user_id_from_token', side_effect=Exception("Invalid token")):
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_deck_not_found(self, mock_websocket, mock_deck_service):
        """Test WebSocket endpoint when deck not found."""
        deck_id = uuid4()
        mock_deck_service.get_deck.side_effect = DeckNotFoundException("Deck not found")
        
        with patch('app.api.websocket.security_service.extract_user_id_from_token', return_value="test-user-123"):
            from app.api.websocket import websocket_endpoint
        ...
        mock_websocket.close.assert_called_with(code=4003, reason="Deck with ID Deck not found not found")

    @pytest.mark.asyncio
    async def test_websocket_endpoint_unauthorized_access(self, mock_websocket, mock_deck_service):
        """Test WebSocket endpoint when user unauthorized."""
        deck_id = uuid4()
        mock_deck_service.get_deck.side_effect = UnauthorizedAccessException("Access denied")
        ...
        mock_websocket.close.assert_called_with(code=4003, reason="User None is not authorized to access Access denied")
...
class TestEventReplay:
    ...
    async def test_send_event_replay_success(self, mock_websocket, sample_events):
        ...
        # Assertions
        mock_deck_service.get_deck_events.assert_called_once_with(deck_id, user_id, from_version)
...
class TestClientMessageHandling:
    ...
    async def test_handle_slide_update_message(self, mock_websocket):
        ...
        with patch('app.api.websocket.connection_manager', mock_connection_manager):
            await _handle_client_message(
                mock_websocket, message_data, deck_id, user_id, 
                mock_deck_service, mock_slide_service
            )
    ...
    async def test_handle_add_slide_message(self, mock_websocket):
        ...
        message_data = json.dumps({
            ...
            "data": {
                "position": 3,
                "prompt": "Add a slide about market analysis"
            }
        })
