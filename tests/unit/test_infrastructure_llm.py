...
class TestLLMClient:
    ...
    async def test_generate_deck_plan_llm_error(self, llm_client):
        ...
        llm_client.client = Mock()
        llm_client._invoke_chain_with_retry = AsyncMock(side_effect=Exception("API error"))
    ...
    @pytest.mark.asyncio
    async def test_generate_slide_content_success(self, llm_client, mock_slide_response):
    ...
    @pytest.mark.asyncio
    async def test_generate_slide_content_without_notes(self, llm_client, mock_slide_response):
    ...
    def test_prompts_have_expected_placeholders_deck_plan(self):
        ...
        assert vars_ == {"title", "topic", "audience", "slide_count", "style", "language"}
    ...
    async def test_generate_deck_plan_passes_expected_inputs(self, llm_client):
        ...
        expected = DeckPlan(title="T", slides=[], total_slides=0, estimated_duration=0, target_audience="A")
        with patch.object(llm_client, "_invoke_chain_with_retry", return_value=expected) as spy:
            await llm_client.generate_deck_plan(
                title="T", topic="Top", audience="Aud", slide_count=3, style="pro", language="en"
            )
        args, kwargs = spy.call_args
        input_dict = args[1]
        assert set(input_dict.keys()) == {"title", "topic", "audience", "slide_count", "style", "language"}
    ...
    async def test_generate_slide_content_passes_expected_inputs(self, llm_client):
        ...
        llm_client.client = Mock()
        llm_client.client.with_structured_output = Mock(return_value=Mock())
        expected = SlideContent(title="t", content="c", html_content="<h1>t</h1>", presenter_notes="n", slide_number=1)
        with patch.object(llm_client, "_invoke_chain_with_retry", return_value=expected) as spy:
            await llm_client.generate_slide_content(
                slide_info={"title": "Intro", "type": "title", "key_points": ["a", "b"], "content_type": "text"},
    ...
    async def test_update_slide_content_passes_expected_inputs(self, llm_client):
        ...
        llm_client.client = Mock()
        llm_client.client.with_structured_output = Mock(return_value=Mock())
        expected = SlideContent(title="t2", content="c2", html_content="<h1>u</h1>", presenter_notes="n2", slide_number=1)
        with patch.object(llm_client, "_invoke_chain_with_retry", return_value=expected) as spy:
        ...
        args, kwargs = spy.call_args
        input_dict = args[1]
        assert set(input_dict.keys()) == {"current_content", "update_prompt", "slide_context"}
