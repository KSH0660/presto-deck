...
class TestPostgresDeckRepositoryIntegration:
    ...
    async def test_deck_crud_operations(self, db_session, test_user_id):
        ...
        assert retrieved_updated.deck_plan == {
            "title": "Updated",
    ...
    async def test_get_by_user_id_with_pagination(
        self, db_session, test_user_id, another_user_id
    ):
...
class TestPostgresSlideRepositoryIntegration:
    ...
    async def test_slide_crud_operations(self, db_session, sample_deck):
        ...
        assert (
            created_slide.html_content
            == "<h1>Integration Test Slide</h1><p>Content</p>"
        )
        ...
        assert (
            updated_slide.html_content == "<h1>Updated Slide</h1><p>Updated content</p>"
        )
        ...
        assert (
            retrieved_updated.html_content
            == "<h1>Updated Slide</h1><p>Updated content</p>"
        )
...
class TestPostgresEventRepositoryIntegration:
    ...
    async def test_get_by_deck_id_with_from_version(self, db_session, sample_deck):
        ...
        all_events = await event_repository.get_by_deck_id(
            sample_deck.id, from_version=0
        )
        assert len(all_events) == 5

        # Test with from_version = 2 (get versions > 2)
        filtered_events = await event_repository.get_by_deck_id(
            sample_deck.id, from_version=2
        )
        ...
        no_events = await event_repository.get_by_deck_id(
            sample_deck.id, from_version=5
        )
        assert len(no_events) == 0

        # Test with from_version beyond max
        beyond_events = await event_repository.get_by_deck_id(
            sample_deck.id, from_version=10
        )
