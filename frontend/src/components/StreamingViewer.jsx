...
const StreamingViewer = ({ topic, quality = 'default', ordered = false, onCompleted, onReset }) => {
  const [slides, setSlides] = useState([]); // may include null placeholders if ordered
        ...
        while (true) {
          ...
          bufferRef.current = parseSSE(bufferRef.current, (type, data) => {
            if (type === 'started') {
              ...
              if (ordered && totalSlides > 0) {
                const placeholders = new Array(totalSlides).fill(null);
                setSlides(placeholders);
                slidesRef.current = placeholders.slice();
              }
            } else if (type === 'deck_plan') {
              setDeckPlan(data);
            } else if (type === 'slide_rendered') {
              const slideObj = { slide_id: data.slide_id, template_name: data.template_name, html: data.html };
              setSlides((prev) => {
                let next;
                if (ordered) {
                  next = prev.slice();
              ...
              let finalSlides = slidesRef.current;
  ...
  const completed = progress.completed || slides.length;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-start p-4 overflow-auto">
      <div className="w-full max-w-6xl mb-4">
        ...
        <div className="text-xs text-gray-400 mt-1">Order: {ordered ? 'Deck-order' : 'Fastest-first'}</div>
