...
class ResourceService:
    ...
    async def _fetch_url_content(self, url: str) -> Optional[str]: # 이부분은 openai deepresearch 를 쓰면 될 듯?! 유로 유저 기준 n 번만 쓸 수 있도록 하는 로직 필요해보임. 
    ...
    async def gather_and_enrich_resources(
        ...
        if user_provided_text:
            gathered_resources.append(GatheredResource(source="user_text", content=user_provided_text))
        ...
        if user_provided_urls:
            ...
            for url, content in zip(user_provided_urls, url_contents):
                if content:
                    gathered_resources.append(GatheredResource(source="user_url", content=content, url=url))
        ...
        if not all_raw_content:
            ...
            llm_response = await generate_content_openai(llm_generated_content_prompt, model=model)
            
            # Assuming llm_response is a string or has a 'text' attribute
            generated_text = llm_response.text if hasattr(llm_response, 'text') else str(llm_response)
            
            gathered_resources.append(GatheredResource(source="llm_generation", content=generated_text))
        ...
        concatenated_summary = "\n\n".join(all_raw_content) if all_raw_content else f"No specific resources found for topic: {topic}."

        return EnrichedContext(
            summary=concatenated_summary,
            raw_resources=gathered_resources)