...
class PresentationService:
    def __init__(self, template_service: TemplateService, resource_service: ResourceService): # Modified
    ...
    async def _generate_outline(self, request: PresentationRequest, available_layouts: list[str], enriched_context: str) -> PresentationOutline:
    ...
    async def _generate_slide_content(self, request: PresentationRequest, plan: dict, generated_slides_history: list, slide_outline: dict, enriched_context: str) -> SlideContent:
    ...
    async def generate_slides_stream(self, request: PresentationRequest) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            enriched_context_obj = await self._gather_and_enrich_resources(request) # Changed
            enriched_context_summary = enriched_context_obj.summary # Get the summary string
            print(f"{enriched_context_summary=}")
            
            # 2. Generate the high-level outline
            outline = await self._generate_outline(request, available_layouts, enriched_context_summary) # Pass summary
            ...
            for i, slide_outline in enumerate(slides_outline):
                ...
                slide_content = await self._generate_slide_content(request, plan, generated_slides_history, slide_outline_dict, enriched_context_summary) # Pass summary
